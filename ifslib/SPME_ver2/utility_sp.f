c---------------------------------------------------------------------
      subroutine spme_direct_sp(numatoms,x,y,z,cg,cutoff,ewaldcof,box,
     $      ene,fx,fy,fz,virial,flg_minimg,spltbl_int,spltbl)
      implicit none
      integer numatoms,nreps(3)
      integer*8 iter_half
      double precision x(*),y(*),z(*),cg(*),cutoff,ewaldcof,box(3),
     $      ene,fx(numatoms),fy(numatoms),fz(numatoms),virial(6)
      double precision flg_minimg(*),spltbl_int,spltbl(*)
      double precision virial_tmp(6),cutoff2
     ...
     &          * int((nreps(3)*2+1),8) / 2
      ...
      if(min(box(1),box(2),box(3)).lt.2.0d0*cutoff) then  !!!!!!!!!!!!
        if(min(box(1),box(2),box(3)).lt.cutoff) then
          call get_dir_pair_ii_sp(numatoms,cg,cutoff2,box,
     $         ene,virial,nreps,iter_half,spltbl_int,spltbl)
        end if
        if(nflg.gt.1) then
...
!$omp     do reduction(+:ene,fx,fy,fz) schedule(dynamic,1)
          do j = 1,numatoms-1
          do i = j+1,numatoms
            call get_dir_pair_sp(numatoms,i,j,x,y,z,cg,
     $           cutoff2,box,ene,fx,fy,fz,virial_tmp,
     $           flg_minimg,nreps,iter_half,spltbl_int,spltbl)
          end do
          end do
!$omp     critical
          do i=1,6
...
!$omp     do reduction(+:ene,fx,fy,fz) schedule(dynamic,1)
          do j = 1,numatoms-1
          do i = j+1,numatoms
            call get_dir_pair_lt15_sp(numatoms,i,j,x,y,z,cg,
     $           cutoff2,box,ene,fx,fy,fz,virial_tmp,
     $           flg_minimg,nreps,iter_half,spltbl_int,spltbl)
          end do
          end do
...
!$omp     do reduction(+:ene,fx,fy,fz) schedule(dynamic,1)
          do j = 1,numatoms-1
          do i = j+1,numatoms
            call get_dir_pair_minimg_sp(numatoms,i,j,x,y,z,cg,
     $           cutoff2,box,ene,fx,fy,fz,virial_tmp,
     $           flg_minimg,spltbl_int,spltbl)
          end do
          end do
!$omp     critical
          do i=1,6
...
!$omp     do reduction(+:ene,fx,fy,fz) schedule(dynamic,1)
          do j = 1,numatoms-1
          do i = j+1,numatoms
            call get_dir_pair_minimg_lt15_sp(numatoms,i,j,x,y,z,cg,
     $           cutoff2,box,ene,fx,fy,fz,virial_tmp,
     $           flg_minimg,spltbl_int,spltbl)
          end do
          end do
...
c---------------------------------------------------------------------
      subroutine get_dir_pair_sp(numatoms,i,j,x,y,z,cg,cutoff2,
     $      box,ene,fx,fy,fz,virial,
     &      flg_array,nreps,iter_half,spltbl_int,spltbl)
      implicit none
      integer numatoms,i,j,nreps(3)
      integer*8 iter_half
      double precision x(*),y(*),z(*),cg(*),cutoff2,box(3),
     $      ene,fx(*),fy(*),fz(*),virial(6)
      double precision flg_array(numatoms,*),spltbl_int,spltbl(*)
       ...
       do 45 l = -nreps(2),nreps(2)
        ...
        do 40 k = -nreps(1),nreps(1)
         ...
         if ( r2 .lt. cutoff2 )then
          ...
          call ew_direct_sp(r,pot,grad_term,flg,spltbl_int,spltbl)
          ene = ene + cg_ij*pot
...
c---------------------------------------------------------------------
      subroutine get_dir_pair_lt15_sp(numatoms,i,j,x,y,z,cg,cutoff2,
     $      box,ene,fx,fy,fz,virial,
     &      flg_array,nreps,iter_half,spltbl_int,spltbl)
      implicit none
      integer numatoms,i,j,nreps(3)
...
c---------------------------------------------------------------------
      subroutine get_dir_pair_minimg_sp(numatoms,i,j,x,y,z,cg,
     $    cutoff2,box,ene,fx,fy,fz,virial,flg_array,spltbl_int,spltbl)
      implicit none
...
c---------------------------------------------------------------------
      subroutine get_dir_pair_minimg_lt15_sp(numatoms,i,j,x,y,z,cg,
     $    cutoff2,box,ene,fx,fy,fz,virial,flg_array,spltbl_int,spltbl)
      implicit none
      integer numatoms,i,j
      double precision x(*),y(*),z(*),cg(*),cutoff2,box(3),
...
c---------------------------------------------------------------------
      subroutine get_dir_pair_ii_sp(numatoms,cg,cutoff2,
     $      box,ene,virial,nreps,iter_half,spltbl_int,spltbl)
      implicit none
      integer numatoms,nreps(3)
      integer*8 iter_half
      double precision cg(*),cutoff2,box(3),
     $      ene,virial(6),spltbl_int,spltbl(*)
      ...
      do 50 m = -nreps(3),nreps(3)
       ...
       do 45 l = -nreps(2),nreps(2)
        ...
        do 40 k = -nreps(1),nreps(1)
         ...
         if ( r2 .lt. cutoff2 )then
          r = sqrt(r2)
          call ew_direct_sp(r,pot,grad_term,1.0d0,spltbl_int,spltbl)
          ene_tmp = ene_tmp + pot
...
c---------------------------------------------------------------------
      subroutine ew_direct_sp(r,pot,term,flg,spltbl_int,spltbl)
      implicit none
      double precision r,pot,term,flg,spltbl_int,spltbl(*)

      double precision merfc,app_erf,app_exp

      call spline_interpfunc(r,app_erf,app_exp,spltbl_int,spltbl)

      merfc  = flg + app_erf
      pot = merfc/r
      term = -(app_exp + pot)/r/r

      return
      end
c---------------------------------------------------------------------
      subroutine spline_interpfunc(r,app_erf,app_exp,spltbl_int,spltbl)
      implicit none
      double precision r,app_erf,app_exp,spltbl_int,spltbl(0:8,0:*)
      integer index_1
      double precision d

      index_1 = INT(r/spltbl_int)
      d = r - spltbl(0,index_1)

        app_erf =    spltbl(4,index_1) + d*(spltbl(3,index_1)
     &          + d*(spltbl(2,index_1) + d* spltbl(1,index_1)))
        app_exp =    spltbl(8,index_1) + d*(spltbl(7,index_1)
     &          + d*(spltbl(6,index_1) + d* spltbl(5,index_1)))

      return
      end
c---------------------------------------------------------------------
      subroutine nsp_to_spltbl_int(nsp,cutoff,spltbl_int)
      implicit real*8(a-h,o-z)

      spltbl_int = cutoff / DBLE(nsp-2)

      return
      end
c---------------------------------------------------------------------
      subroutine spltbl_int_to_nsp(nsp,cutoff,spltbl_int)
      implicit real*8(a-h,o-z)

      nsp = nint(cutoff / spltbl_int) + 2 
