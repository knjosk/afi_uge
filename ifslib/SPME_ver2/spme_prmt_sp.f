c---------------------------------------------------------------------
      subroutine spme_prmt_sp(numatoms,x,y,z,cg,flg_minimg,box,
     $     tolerance_org,cutoff_min,cutoff_best,ewaldcof_best,
     $     iorder_best,nfft1_best,nfft2_best,nfft3_best,nsp_best,
     ...
     &          iflag(5,-jmax:jmax)
      dimension calib_cutoff(5*2*jmax),
C    &          ncalib_nsp(5*2*jmax),
     &          calib_time(5*2*jmax),
     &          i_calib_cutoff(5*2*jmax),
     &          i_calib_time(5*2*jmax),
     &          ii_store(2*jmax),
     &          jj_store(2*jmax),
     &          i_best(-jmax:jmax)
      real*4    etime,tm(2)
...
C       write(*,'(a,i5)')    'j = ',j
        ...
        call find_prmt_sp(numatoms,x,y,z,cg,flg_minimg,box,recip,
     $     volume,
     $     tolerance,cutoff_min,cutoff(1,j),ewaldcof(j),iorder(1,j),
     $     nfft1(1,j),nfft2(1,j),nfft3(1,j),nsp(1,j),
     ...
     $     cutoff_ic,k1_ltd,k2_ltd,k3_ltd,spdr_base_ic,
...
C       write(11,'(i5,e12.4,3i4)') j,ewaldcof(j),(nerrors(i,j),i=1,3)
        if(ttl_time(1,j).gt.1.0d99) then
          if(j.eq.0) then
            ...
            print *,'       Try the subroutine ''spme_prmt''.'
            stop
          else
...
C           write(*,*) 'ewaldcof. exit from the first do-while loop.'
        ...
        do i=1,5
          nsp_tmp = min(nsp(i,j)-1, int(DBLE(nsp(i,j)-2)/1.01d0) + 3)
          cutoff_tmp = cutoff(i,j) * DBLE(nsp_tmp-2)/DBLE(nsp(i,j)-2)
C         if(cutoff_tmp .le. cutoff_min) then
C           write(*,'(a,i2,i6,e12.4,i6,e12.4)') 
C    &                         '**** cutoff_tmp .le. cutoff_min  ',i,
C    &                         nsp(i,j),cutoff(i,j),nsp_tmp,cutoff_tmp
C         end if
          if(cutoff_tmp .gt. cutoff_min .or. 
     &       nsp(i,j) .le. nsp_min+1 ) n_cutoff_min = 0
        end do
        n_cutoff_min = n_cutoff_min + 1
C       write(*,'(a,i2)') '**** n_cutoff_min = ', n_cutoff_min
C       write(*,*)
...
C     do j=js,je
C       write(10,'(i5,2e12.4,4i5,i6,3e15.7)')j,ewaldcof(j),
C    &            cutoff(1,j),
C    &            iorder(1,j),nfft1(1,j),
C    &            nfft2(1,j),nfft3(1,j),nsp(1,j),
...
C     do n=1,ndata
C       write(*,'(i6,i5,a,e10.4,i5,a,i6,a,e13.7,i5,a)') 
C    &      ncalib_nsp(n),n,':',calib_cutoff(n),i_calib_cutoff(n),':',
...
C       write(*,'(i5,2e12.4,4i5,i6,3e15.7,i4)')j,ewaldcof(j),
C    &            cutoff(i,j),
C    &            iorder(i,j),nfft1(i,j),
C    &            nfft2(i,j),nfft3(i,j),nsp(i,j),
...
C       write(8,'(i5,2e12.4,4i5,i6,3e15.7)')j,ewaldcof(j),
C    &            cutoff(i_best(j),j),
...
C       write(*,'(i5,2e12.4,4i5,i6,3e15.7)')j,ewaldcof(j),
C    &            cutoff(i_best(j),j),
C    &            iorder(i_best(j),j),nfft1(i_best(j),j),
C    &            nfft2(i_best(j),j),nfft3(i_best(j),j),
C    &            nsp(i_best(j),j),
...
C     write(*,*) time_self
      ...
      nsp_best = nsp(i_best(j_best),j_best)
...
c---------------------------------------------------------------------
      subroutine find_prmt_sp(numatoms,x,y,z,cg,flg_minimg,box,
     $     recip,volume,
     $     tolerance,cutoff_min,cutoff_best,ewaldcof,iorder_best,
     $     nfft1_best,nfft2_best,nfft3_best,nsp_best,tol_ene_best,
     ...
     &           nsp_max=100000,nsp_min=2,nsp_exact_min=64,
     &           iorder_dummy=4,nfft_dummy=8)
      dimension x(*),y(*),z(*),cg(*),flg_minimg(*),box(3),Q(*),
     $      iorder_choice(iorder_nchoice),nfft_choice(nfft_nchoice),
     $      cutoff_best(5),iorder_best(5),
     $      nfft1_best(5),nfft2_best(5),nfft3_best(5),
     $      nsp_best(5),tol_ene_best(5),
     ...
     &          nfft3(5,5),nsp(5),
     &          rec_time(5,5),dir_time(5),ttl_time(25)
     ...
     &          dir_time_cal(5),i_dir_time_cal(5)
      dimension bsp_mod_dummy(nfft_dummy,3)
      dimension fftable_dummy(2*3*nfft_dummy+256*3)
      dimension spltbl(0:8,0:nsp_max-1)
...
c     write(*,'(e12.4,2x,a)') ewaldcof,'1.1'
      ...
      cutoff_exact = max(cutoff_exact,cutoff_min)
...
c     cutoff_ic = max(cutoff_ic,cutoff_min)

      if(spdr_base_ic.eq.0.0d0) then
        spdr_base_ic = cutoff_exact / 100.0d0
        spdr_base_ic = log10(spdr_base_ic)
        int_spdr_base_ic = nint(spdr_base_ic/0.1d0)
        spdr_base_ic = DBLE(int_spdr_base_ic)*0.1d0
        spdr_base_ic = 10.0d0**spdr_base_ic
C       write(*,*)
C       write(*,*) 'spdr_base_ic = 0 and its temporary value is set '
C       write(*,*) 'to be ', spdr_base_ic
        call spltbl_int_to_nsp(nsp_exact,cutoff_exact,spdr_base_ic)
        nsp_exact = min(nsp_exact,nsp_max)
        call spme_init_sp(numatoms,iorder_dummy,nfft_dummy,
     $           nfft_dummy,nfft_dummy,bsp_mod_dummy,fftable_dummy,
     $           cutoff_exact,ewaldcof,nsp_exact,spltbl_int,spltbl)
        call find_dir_error_sp(numatoms,x,y,z,cg,cutoff_exact,
     $                    ewaldcof,box,flg_minimg,spltbl_int,spltbl,
     ...
     &          tolvir_tmp(1),time_tmp,1)
        tolxyz(1) = (tolx_tmp(1)+toly_tmp(1)+tolz_tmp(1)) / 3.0d0
        nstep_gap = int((dtol_exp_sp - tolxyz(1))/0.4d0)*25
c--------------'0.4d0' in the previous line was obtained empirically.
C       write(*,*) 'current precision is ', tolxyz(1)
C       write(*,*) 'dtol_exp_sp = ', dtol_exp_sp
C       write(*,*) 'nstep_gap = ', nstep_gap
        call next_value(spdr_base_ic,nstep_gap)
C       write(*,*) 'taking the required precision into account, '
C       write(*,*) 'initial spdr_base_ic is finally ',spdr_base_ic
      end if

c----2.15 find the largest spdr_base_ic that satisfies the precision 
c---------dtol_exp_sp when cutoff == cutoff_exact for the given 
c---------ewaldcof. With the new spdr_base_ic, overwrite the current 
c---------spdr_base_ic that currenly has a reference value, and then, 
c---------calculate spltbl_int and spltbl from the new spdr_base_ic. 
c     write(*,'(e12.4,2x,a)') ewaldcof,'2.15'
      call spltbl_int_to_nsp(nsp_exact,cutoff_exact,spdr_base_ic)
      nsp_exact = min(nsp_exact,nsp_max)
      nsp_exact = max(nsp_exact_min,nsp_exact)
      call spme_init_sp(numatoms,iorder_dummy,nfft_dummy,
     $           nfft_dummy,nfft_dummy,bsp_mod_dummy,fftable_dummy,
     $           cutoff_exact,ewaldcof,nsp_exact,spltbl_int,spltbl)
      call find_dir_error_sp(numatoms,x,y,z,cg,cutoff_exact,
...
C     write(*,*) 
C     write(*,'(a,e12.4)') 'cutoff_exact = ', cutoff_exact
C     write(*,'(a,f10.5)') 'dtol_exp_sp = ', dtol_exp_sp
C     write(*,'(a,2e12.4,i7,f10.5)') 
C    &           'searching new spdr_base_ic ...',
C    &            spdr_base_ic,spltbl_int,nsp_exact,tolxyz(1)
      if (tolxyz(1).gt.dtol_exp_sp) then
        do while(tolxyz(1).gt.dtol_exp_sp)
          call next_value(spdr_base_ic,-25)
          call spltbl_int_to_nsp(nsp_exact,cutoff_exact,spdr_base_ic)
          if(nsp_exact.gt.nsp_max) then
C           print *,'impossible to achieve such precision! (sp1)'
C           print *,'try loosening the required tolerance.'
C           print *,ewaldcof,tolxyz(1),dtol_exp_sp,nsp_exact,nsp_max
            go to 801
          end if
          call spme_init_sp(numatoms,iorder_dummy,nfft_dummy,
     $           nfft_dummy,nfft_dummy,bsp_mod_dummy,fftable_dummy,
...
C    &                spdr_base_ic,spltbl_int,nsp_exact,tolxyz(1)
      ...
      else if (tolxyz(1).lt.dtol_exp_sp .and. 
     &         nsp_exact.gt.nsp_exact_min) then
        do while(tolxyz(1).lt.dtol_exp_sp)
          spdr_base_ic_old = spdr_base_ic
          call next_value(spdr_base_ic, 25)
          call spltbl_int_to_nsp(nsp_exact,cutoff_exact,spdr_base_ic)
          if(nsp_exact.lt.nsp_exact_min) then
C           write(*,*) 'nsp_exact.lt.nsp_exact_min exit from the loop'
            exit
          end if
          call spme_init_sp(numatoms,iorder_dummy,nfft_dummy,
     $           nfft_dummy,nfft_dummy,bsp_mod_dummy,fftable_dummy,
...
C    &                spdr_base_ic,spltbl_int,nsp_exact,tolxyz(1)
        ...
        call spltbl_int_to_nsp(nsp_exact,cutoff_exact,
     &                         spdr_base_ic_old)
        call spme_init_sp(numatoms,iorder_dummy,nfft_dummy,
     $           nfft_dummy,nfft_dummy,bsp_mod_dummy,fftable_dummy,
     $           cutoff_exact,ewaldcof,nsp_exact,spltbl_int,spltbl)
        spdr_base_ic = spdr_base_ic_old
      end if
C     write(*,*) 'spdr_base_ic has now been determined!'
C     write(*,*) 'and so have spltbl_int and spltbl.'
C     write(*,*) 'spdr_base_ic = ',spdr_base_ic
C     write(*,*) 'spltbl_int = ',spltbl_int
C     write(*,*) 'nsp_exact = ',nsp_exact
C     write(*,*)

c----2.17 adjust cutoff_ic to the closest node of the spline.
      call spltbl_int_to_nsp(nsp_ic,cutoff_ic,spltbl_int)
C     write(*,*) 'original cutoff_ic = ',cutoff_ic
C     write(*,*) 'original nsp_ic = ',nsp_ic
      nsp_ic = min(nsp_ic,nsp_exact)
      cutoff_ic = spltbl(0,nsp_ic-2)
      do while(cutoff_ic.lt.cutoff_min)
        nsp_ic = nsp_ic + 1
        if(nsp_ic.gt.nsp_exact) then
C         print *,'failed to obtain cutoff_ic.'
C         print *,'cutoff_ic is below cutoff_min but nsp_ic is '
C         print *,'already .gt. nsp_exact!' 
C         print *,ewaldcof,cutoff_ic,cutoff_min,nsp_ic,spltbl_int
          go to 801
        end if
        cutoff_ic = spltbl(0,nsp_ic-2)
      end do
C     write(*,*) 'new nsp_ic = ',nsp_ic
C     write(*,*) 'spltbl(0,nsp_ic-3) = ',spltbl(0,nsp_ic-3)
C     write(*,*) 'spltbl(0,nsp_ic-2) = ',spltbl(0,nsp_ic-2),
C    &           ' <- new cutoff_ic'
C     write(*,*) 'spltbl(0,nsp_ic-1) = ',spltbl(0,nsp_ic-1)
C     write(*,*)

c----------REMEMBER : cutoff_exact = spltbl(0,nsp_exact-2)
...
c     write(*,'(e12.4,2x,a)') ewaldcof,'2.3'
      call find_dir_error_sp(numatoms,x,y,z,cg,cutoff_ic,
     $                    ewaldcof,box,flg_minimg,spltbl_int,spltbl,
     $          dene_tmp,dfx_tmp,dfy_tmp,dfz_tmp,dvir_tmp,self_ene,
     $          rene_exact,rfx_exact,rfy_exact,rfz_exact,rvir_exact,
     $          ene_exact,fx_exact,fy_exact,fz_exact,vir_exact,
...
C     write(*,'(i2,i7,e12.4,e15.7,2f10.5)')
C    &            0,nsp_ic,cutoff_ic,dir_time_ic,dtol_exp(5),tolxyz(1)
...
c     write(*,'(e12.4,2x,a)') ewaldcof,'2.4'
      ...
      do while(flag(5).gt.0.0d0)
        nsp_tmp = max(nsp_tmp+1, int(DBLE(nsp_tmp-2) * 1.01d0) + 2)
        if(nsp_tmp.ge.nsp_exact) then
C         print *,'examining nsp_tmp is now .ge. nsp_exact.'
          if(ie.eq.0 .and. flag(1).gt.0.0d0) then
C           print *,'impossible to achieve such precision! (dir1)'
C           print *,'try loosening the required tolerance.'
C           print *,ewaldcof,nsp_tmp,cutoff_tmp,tolxyz(1)
            go to 801
          else
...
C           print *,'for all 5 dtol_exp! (dir1)'
        ...
        cutoff_tmp = spltbl(0,nsp_tmp-2)
        call find_dir_error_sp(numatoms,x,y,z,cg,cutoff_tmp,
     $                    ewaldcof,box,flg_minimg,spltbl_int,spltbl,
     $          dene_tmp,dfx_tmp,dfy_tmp,dfz_tmp,dvir_tmp,self_ene,
     $          rene_exact,rfx_exact,rfy_exact,rfz_exact,rvir_exact,
     $          ene_exact,fx_exact,fy_exact,fz_exact,vir_exact,
     ...
     &          tolvir_tmp(1),time_tmp,nrepeats)
        ...
        tolxyz(1) = max(tolx_tmp(1),toly_tmp(1),tolz_tmp(1))
C       print *,nsp_tmp,cutoff_tmp,tolxyz(1)
...
C           write(*,'(i2,i7,e12.4,e15.7,2f10.5)')
C    &            i,nsp(i),cutoff(i),dir_time(i),dtol_exp(i),tolxyz(1)
            ie = i
          end if
        end do
      end do
...
!$omp   parallel do
        ...
        nsp_tmp = min(nsp_tmp-1, int(DBLE(nsp_tmp-2) / 1.01d0) + 3)
        cutoff_tmp = spltbl(0,max(0,nsp_tmp-2))
                      if(cutoff_tmp.lt.cutoff_min .or. 
     &                   nsp_tmp.le.nsp_min) then
                        do i=1,5
...
!$omp                       parallel do
                            ...
                            ie = max(ie,i)
C                       write(*,'(i2,i7,e12.4,e15.7)') 
C    &                                  i,nsp(i),cutoff(i),dir_time(i)
C                       write(*,*)'best cutoff .lt. cutoff_min .or. ',
C    &                                         'nsp_tmp .le. nsp_min.'
                          end if
                        end do
                        go to 601
                      end if
...
C           do ii=1,ie
C             write(*,'(i3,i7,3e12.4)') ii,nsp(ii),cutoff(ii),
...
c           nsp_ic = max(nsp_min + 1, nsp(1) - 1)
            nsp_ic = min(nsp(1)-1, int(DBLE(nsp(1)-2) / 1.01d0) + 3)
            nsp_ic = max(nsp_ic,nsp_min+1)
          else
c           nsp_ic = min(nsp_exact, nsp(ie) + 1)
            nsp_ic = max(nsp(ie)+1, int(DBLE(nsp(ie)-2) * 1.01d0) + 2)
            nsp_ic = min(nsp_ic,nsp_exact)
          end if
          cutoff_ic = spltbl(0,nsp_ic-2)
          if(cutoff_ic.lt.cutoff_min) then
            nsp_ic = nsp_ic + 1
            cutoff_ic = spltbl(0,nsp_ic-2)
          end if
          go to 599
...
C     do i=1,5
C       do j=1,5
C         write(*,'(2i4,i7,e12.4,4i5,3e13.5)')
C    &         i,j,nsp(i),cutoff(i),iorder(j,i),nfft1(j,i),
...
C     write(*,*)'-----------------------------------'
     ...
     &     nfft3_best(l-1) .ne. nfft3(j,i) .or.
     &     nsp_best(l-1) .ne. nsp(i)) then
             cutoff_best(l) = cutoff(i)
             iorder_best(l) = iorder(j,i)
             nfft1_best(l) = nfft1(j,i)
             nfft2_best(l) = nfft2(j,i)
...
C     do l=1,5
C         write(*,'(i7,e12.4,4i5,e15.7)')
C    &         nsp_best(l),cutoff_best(l),iorder_best(l),
C    &         nfft1_best(l),nfft2_best(l),nfft3_best(l),
...
c---------------------------------------------------------------------
      subroutine find_dir_error_sp(numatoms,x,y,z,cg,cutoff,
     $                    ewaldcof,box,flg_minimg,spltbl_int,spltbl,
     ...
     &               tolene,tolx,toly,tolz,tolvir,time,nrepeats)
      ...
      dimension x(*),y(*),z(*),cg(*),flg_minimg(*),box(3),spltbl(*)
...
c     do while((t2-t1).lt.DBLE(nrepeats)*0.5d0)
      do while((t2-t1).lt.0.2d0)
        do i=1,nrepeats
          call spme_direct_sp(numatoms,x,y,z,cg,cutoff,ewaldcof,box,
     $        dene,dfx,dfy,dfz,dvir,flg_minimg,spltbl_int,spltbl)
        end do
        n = n + 1
        t2 = etime(tm)
      end do
