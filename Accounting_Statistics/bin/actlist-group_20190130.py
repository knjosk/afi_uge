...
Usage:
    {f} [<group_name>] [<group_used_py.csv>]
    {f} -h | --help
...
Options:
    ...
    [<group_used_py.csv>]   User Accounting file.(not requierd)
...
def parse():
    ...
    group_usage_file_name = "/opt/uge/Accounting_Statistics/logs/accounting/group_used_py.csv"
    if args['<group_used_py.csv>']:
        group_usage_file_name = args['<group_used_py.csv>']
    ...
    if group_name + "-s" in group_usage_dict:
        ...
        print("--------------TOTAL NODE HOURS--------------------------------------")
        print('[Group Name -d  :{:>15}]'.format(group_name + "-d"))
