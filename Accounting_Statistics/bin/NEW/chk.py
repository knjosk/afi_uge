...
def parse():
    ...
    if ((D_USER == False and D_PRJ == False) or (D_USER == True and D_ADMIN_G == True)) and D_MONTH == False and D_FY == False:
        ...
        start_day = (FiscalYear(this_year).start).strftime('%Y/%m/%d')
        ...
        user_gpu_usage_file_name = "/opt/uge/Accounting_Statistics/logs/accounting/user_gpu_used_py.csv"
        if os.path.exists(user_gpu_usage_file_name) != True:
            print "User GPU Usage Accounting file not exist."
            sys.exit()
        user_gpu_usage_f = open(user_gpu_usage_file_name, 'r')
        reader = csv.reader(user_gpu_usage_f)
        for row in reader:
            user_gpu_usage_dict[row[0]] = [float(row[1]), float(row[2]), float(row[3]), float(row[4])]

        if user_name not in user_gpu_usage_dict:
            user_gpu_usage_dict[user_name] = [0, 0, 0, 0]
    ...
    elif D_MONTH == False and D_PRJ == True and (D_SAME_PRJ == True or D_ADMIN_G == True) and D_FY == False:
    ...
    elif D_FY == True and (D_USER == False or D_SAME_USER == True or D_ADMIN_G == True) and D_PRJ == False:
        set_fy(target_year)
        now=datetime.datetime.now()
        month=now.strftime('%Y%m')
        target_month=""
        i=m=0
        while m != 3 and target_month != month:
            target_month=(FiscalYear(target_year).start + relativedelta(months=i)).strftime('%Y%m')
            m=int(target_month[-2:])
            i += 1
            print target_month
            user_usage_file_name_base="/opt/uge/Accounting_Statistics/logs/accounting/user_used_"
            user_usage_file_name=user_usage_file_name_base + 'pm.' + target_month
    ...
    elif D_FY == True and D_PRJ == True and (D_SAME_PRJ == True or D_ADMIN_G == True):
        set_fy(target_year)
        now=datetime.datetime.now()
        month=now.strftime('%Y%m')
        target_month=""
        ...
        while m != 3 and target_month != month:
            ...
            i += 1
            print target_month
            prj_usage_file_name_base = "/opt/uge/Accounting_Statistics/logs/accounting/prj_used_"
            prj_usage_file_name = prj_usage_file_name_base + 'pm.' + target_month
 
            if target_month == month:
                prj_usage_file_name=prj_usaage_file_name_base + 'pm.' + 'csv'
                if os.path.exists(user_usage_file_name) != True:
                    print "Project Usage Accounting file not exist."
                    sys.exit()
            term="Month: " + target_month[:4] + "/" + target_month[-2:]
            prj_usage_f = open(prj_usage_file_name, 'r')
            reader = csv.reader(prj_usage_f)
