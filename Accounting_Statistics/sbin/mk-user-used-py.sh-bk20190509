...
if [ ${YEAR} = "2018" ]; then
    if [ ${MONTH} = "08" ]; then
        ...
        cp ${LOGS_DIR}/user_used_pm.201808 ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
        ...
        for i in $NUM;
        do
            ${UGE_AS_DIR}/sbin/merge2usage.py ${LOGS_DIR}/user_used_pm.${YEAR}$i ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH} ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
        done
        ${UGE_AS_DIR}/sbin/merge2usage.py ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH} ${USER_USED_CURRENR_MONTH_FILE} ${USER_USED_PER_YEAR_FILE}
    fi
else
    if [ ${MONTH} -ge 4 ]; then
        if [ ${MONTH} -eq 4 ]; then
            ...
            cp ${LOGS_DIR}/user_used_pm.${YEAR}04 ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
            ${UGE_AS_DIR}/sbin/merge2usage.py ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH} ${USER_USED_CURRENR_MONTH_FILE} ${USER_USED_PER_YEAR_FILE}
        else
        ...
        if [ ${YEAR} = "2019" ]; then
            if [ ${MONTH} -eq 1 ]; then
                LAST_Y=`expr ${YEAR} - 1`
                NUM=`seq -f %02g 9 12`
                cp ${LOGS_DIR}/user_used_pm.${LAST_Y}08 ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
                for i in $NUM;
                do
            ...
            if [ ${MONTH} -eq 1 ]; then
                LAST_Y=`expr ${YEAR} - 1`
                NUM=`seq -f %02g 5 12`
                cp ${LOGS_DIR}/user_used_pm.${LAST_Y}04 ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
                for i in $NUM;
                do
                    ${UGE_AS_DIR}/sbin/merge2usage.py ${LOGS_DIR}/user_used_pm.${LAST_Y}$i ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH} ${USER_USED_TOTAL_EXCEPT_CURRENT_MONTH}
                done
